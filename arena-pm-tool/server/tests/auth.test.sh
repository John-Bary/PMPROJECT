...
echo "🧪 Testing Arena PM Tool Authentication API"
echo "============================================"
echo ""
...
echo "2️⃣  Testing login (admin@arena.com)..."
...
curl -s -X POST "$BASE_URL/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"wrong@arena.com","password":"wrongpass"}' | json_pp
echo ""
...
curl -s -X POST "$BASE_URL/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"email":"newuser@arena.com","password":"newpass123","name":"New User"}' | json_pp
echo ""
