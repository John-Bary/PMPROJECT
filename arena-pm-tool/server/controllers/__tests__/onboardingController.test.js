...
describe('Onboarding Controller', () => {
  ...
  // -------------------------------------------------------------------
  // safeError — production branch (line 8)
  // -------------------------------------------------------------------
  describe('safeError production branch', () => {
    ...
    it('should hide error details when NODE_ENV is production (line 8 branch)', async () => {
      ...
      const { startOnboarding: prodStartOnboarding } = require('../onboardingController');

      prodQuery.mockRejectedValueOnce(new Error('Sensitive DB error'));

      const prodReq = createMockReq();
      const prodRes = createMockRes();
      prodReq.user = { id: USER_ID };
      prodReq.params = { id: WORKSPACE_ID };

      await prodStartOnboarding(prodReq, prodRes);

      expect(prodRes.status).toHaveBeenCalledWith(500);
      const responseBody = prodRes.json.mock.calls[0][0];
