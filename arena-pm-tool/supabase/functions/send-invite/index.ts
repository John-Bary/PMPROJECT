...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p style="color: #4b5563; font-size: 16px;">
      <strong>${escapeHtml(inviterName)}</strong> has invited you to join <strong>${escapeHtml(workspaceName)}</strong> on Todoria.
    </p>
...
If you didn't expect this invitation, you can safely ignore this email.
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Todoria <noreply@todorio.com>",
        to: [email],
        subject: `You're invited to join ${escapeHtml(workspaceName)} on Todoria`,
        html: emailHtml,
        text: emailText,
      }),
    });
