...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
    <h1 style="color: white; margin: 0; font-size: 24px;">PM Arena</h1>
  </div>
  ...
  <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p style="color: #4b5563; font-size: 16px;">
      <strong>${inviterName}</strong> has invited you to join <strong>${workspaceName}</strong> on PM Arena.
    </p>

    <p style="color: #4b5563; font-size: 16px;">
      PM Arena is a collaborative project management tool that helps teams stay organized and productive.
    </p>
    ...
    // Plain text version
    const emailText = `
You're invited to join ${workspaceName} on PM Arena!

${inviterName} has invited you to collaborate on PM Arena, a project management tool that helps teams stay organized and productive.
...
If you didn't expect this invitation, you can safely ignore this email.
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "PM Arena <noreply@pmarena.com>",
        to: [email],
        subject: `You're invited to join ${workspaceName} on PM Arena`,
        html: emailHtml,
        text: emailText,
      }),
    });
